...
namespace Plumsys.BLL
{
    ...
    public partial class sms_message
    {
        ...
        public bool Send(string mobiles, string content, string templet_code, string sign_name, out string msg)
        {
            //检查是否设置好短信账号
            if (!Exists())
            {
            ...
            for (int i = 0; i < batch; i++)
            {
                ...
                if (sb.ToString().Length > 0)
                {
                    try
                    {
                        ITopClient client = new DefaultTopClient(siteConfig.smsapiurl, siteConfig.smsusername, siteConfig.smspassword);
                        AlibabaAliqinFcSmsNumSendRequest req = new AlibabaAliqinFcSmsNumSendRequest();
                        req.Extend = string.Empty;
                        req.SmsType = "normal" ;
                        req.SmsFreeSignName = sign_name;
                        req.SmsParam = content;
                        req.RecNum = sb.ToString().TrimEnd(',') ;
                        req.SmsTemplateCode = templet_code;

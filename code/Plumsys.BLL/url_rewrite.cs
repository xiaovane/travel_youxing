...
namespace Plumsys.BLL
{
    public class url_rewrite
    {
        ...
        {
            Hashtable ht = CacheHelper.Get<Hashtable>(PLKeys.CACHE_SITE_URLS);
            if (ht == null)
            {
                CacheHelper.Insert(PLKeys.CACHE_SITE_URLS, dal.GetList(), Utils.GetXmlMapPath(PLKeys.FILE_URL_XML_CONFING));
                ht = CacheHelper.Get<Hashtable>(PLKeys.CACHE_SITE_URLS);
            }
            return ht;
        }
        ...
        {
            List<Model.url_rewrite> ls = CacheHelper.Get<List<Model.url_rewrite>>(PLKeys.CACHE_SITE_URLS_LIST);
            if (ls == null)
            {
                CacheHelper.Insert(PLKeys.CACHE_SITE_URLS_LIST, dal.GetList(""), Utils.GetXmlMapPath(PLKeys.FILE_URL_XML_CONFING));
                ls = CacheHelper.Get<List<Model.url_rewrite>>(PLKeys.CACHE_SITE_URLS_LIST);
            }
            return ls;
        }

...
namespace Plumsys.BLL
{
	...
	public partial class channel_site
	{
        ...
        public bool Exists(string build_path)
        {
            ...
            if (DirPathExists(siteConfig.webpath + "/" + PLKeys.DIRECTORY_REWRITE_ASPX + "/", build_path))
            {
                return true;
            }
            //与频道名称是否重名
		...
		public bool Update(Model.channel_site model)
		{
            ...
            if (dal.Update(model, old_build_path))
            {
                ...
                {
                    //更改频道分类对应的目录名称
                    Utils.MoveDirectory(siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_ASPX + "/" + old_build_path,
                        siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                    Utils.MoveDirectory(siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_HTML + "/" + old_build_path,
                        siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                }
                return true;
            }
            return false;

...
namespace Plumsys.DAL
{
    public partial class plugin
    {
        ...
        public Model.plugin GetInfo(string dirPath)
        {
            ...
            if (!File.Exists(dirPath + PLKeys.FILE_PLUGIN_XML_CONFING))
                return aboutInfo;

            XmlDocument xml = new XmlDocument();
            xml.Load(dirPath + PLKeys.FILE_PLUGIN_XML_CONFING);
            try
            {
        ...
        {
            XmlNodeList xnList = XmlHelper.ReadNodes(dirPath + PLKeys.FILE_PLUGIN_XML_CONFING, xPath);
            foreach (XmlElement xe in xnList)
            {
                if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite")
                {
                        ...
                        && !string.IsNullOrEmpty(xe.Attributes["templet"].InnerText) && !string.IsNullOrEmpty(xe.Attributes["inherit"].InnerText))
                    {
                        ...
                        //生成模板文件
                        PageTemplate.GetTemplate(sitePath, tempPath, skinName, xe.Attributes["templet"].InnerText, xe.Attributes["page"].InnerText, xe.Attributes["inherit"].InnerText, PLKeys.DIRECTORY_REWRITE_PLUGIN, string.Empty, pagesizeStr, 1);
                    }
                }
            }
        }
        ...
        {
            return XmlHelper.UpdateNodeInnerText(dirPath + PLKeys.FILE_PLUGIN_XML_CONFING, xPath, value);
        }
        ...
        {
            bool result = true;
            List<string> ls = ReadChildNodesValue(dirPath + PLKeys.FILE_PLUGIN_XML_CONFING, xPath);
            if (ls != null)
            {

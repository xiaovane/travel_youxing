...
namespace Plumsys.DAL
{
    ...
    public partial class area_category
    {
        ...
        public area_category(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from " + databaseprefix + "area_category");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 title from " + databaseprefix + "area_category");
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "area_category set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "area_category ");
        ...
        public DataTable GetParent(int id)
        {
            ...
            strSql.Append("select id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description");
            strSql.Append(" from " + databaseprefix + "area_category");
            strSql.Append(" where id=(select top 1 parent_id from  " + databaseprefix + "area_category where id="+id+") order by sort_id asc,id desc");
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            return ds.Tables[0];
        }
        /// <summary>
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 parent_id from " + databaseprefix + "area_category");
            strSql.Append(" where id=" + id);
        ...
        /// 从内存中取得所有上级类别列表（自身迭代）
        /// </summary>
        private void GetParents(DataTable oldData, DataTable newData, int id)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                newData.Rows.Add(row);
                //调用自身迭代
                this.GetParents(oldData, newData, int.Parse(dr[i]["parent_id"].ToString()));
            }
        }
        /// <summary>
        /// 修改子节点的ID列表及深度（自身迭代）

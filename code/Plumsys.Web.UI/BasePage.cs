...
namespace Plumsys.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        {
            //如果Session为Null
            if (HttpContext.Current.Session[PLKeys.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie(PLKeys.COOKIE_USER_NAME_REMEMBER, "Plumsys");
                string password = Utils.GetCookie(PLKeys.COOKIE_USER_PWD_REMEMBER, "Plumsys");
                if (username != "" && password != "")
                {
                    ...
                    if (model != null)
                    {
                        HttpContext.Current.Session[PLKeys.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        public Model.users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Model.users model = HttpContext.Current.Session[PLKeys.SESSION_USER_INFO] as Model.users;
                if (model != null)
                {
        ...
        private string GetFirstPath(string requestPath)
        {
            ...
            if (requestPath.StartsWith(config.webpath + PLKeys.DIRECTORY_REWRITE_ASPX + "/"))
            {
                indexNum = (config.webpath + PLKeys.DIRECTORY_REWRITE_ASPX + "/").Length;
            }

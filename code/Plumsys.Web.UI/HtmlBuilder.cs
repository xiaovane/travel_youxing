        ...
        private string GetCategoryUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", PLKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", PLKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, dt.Rows[i]["id"].ToString()), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
        ...
        private string GetDetailUrlList(string lang, string channelname, string page, string pattern, string path, string querystring)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", PLKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", PLKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, strvalue), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
                    sburl.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
                }

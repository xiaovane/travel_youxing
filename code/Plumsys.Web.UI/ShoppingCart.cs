...
namespace Plumsys.Web.UI
{
    ...
    public partial class ShopCart
    {
        ...
        /// 根据用户id分组购物车id
        /// </summary>
        public static List<int> GroupListBySellerId(List<Model.cart_items> items)
        {
            List<int> seller_ids =new List<int>();
            foreach (Model.cart_items item in items)
            {
                if (!seller_ids.Contains(item.seller_id)) seller_ids.Add(item.seller_id);
            }
            return seller_ids;
        }
        ...
        {
            Utils.WriteCookie(PLKeys.COOKIE_SHOPPING_CART, "", -43200);
        }
        ...
        {
            Utils.WriteCookie(PLKeys.COOKIE_SHOPPING_CART, strValue, 43200); //存储一个月
        }
        ...
        {
            return Utils.GetCookie(PLKeys.COOKIE_SHOPPING_CART);
        }

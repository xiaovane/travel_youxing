...
namespace Plumsys.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        {
            //如果Session为Null
            if (Session[PLKeys.SESSION_ADMIN_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string adminname = Utils.GetCookie("AdminName", "Plumsys");
                string adminpwd = Utils.GetCookie("AdminPwd", "Plumsys");
                if (adminname != "" && adminpwd != "")
                {
                    ...
                    if (model != null)
                    {
                        Session[PLKeys.SESSION_ADMIN_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        public Model.manager GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Model.manager model = Session[PLKeys.SESSION_ADMIN_INFO] as Model.manager;
                if (model != null)
                {
                    return model;
                }

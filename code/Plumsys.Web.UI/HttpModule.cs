...
namespace Plumsys.Web.UI
{
            ...
            if (siteConfig.staticstatus == 0)
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls)
                {
                    ...
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        if (model.type == PLKeys.DIRECTORY_REWRITE_PLUGIN)
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, PLKeys.DIRECTORY_REWRITE_PLUGIN, requestPage));
                            return;
                        }
                        else
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, sitePath, requestPage));
                            return;
                        }
                    }
                }
            ...
            else
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetUrls().Urls)
                {
                        ...
                        Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, PLKeys.DIRECTORY_REWRITE_PLUGIN, model.page));
                            return;
                        }
                        else
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, sitePath, model.page));
                            return;
                        }
                    }
                    //遍历URL字典的子节点
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPage, string.Format("^/{0}$", item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            ...
                            {
                                context.RewritePath(siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_HTML + "/" + sitePath +
                                        Utils.GetUrlExtension(requestPage, siteConfig.staticextension, true));
                                return;
                            }
                            else if (model.type == PLKeys.DIRECTORY_REWRITE_PLUGIN) //插件页
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}", 
                                    siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, PLKeys.DIRECTORY_REWRITE_PLUGIN, model.page), string.Empty, queryString);
                                return;
                            }
                            else //其它
                            {
                                string queryString = Regex.Replace(requestPage, string.Format("/{0}", newPattern), item.querystring, RegexOptions.None | RegexOptions.IgnoreCase);
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, PLKeys.DIRECTORY_REWRITE_ASPX, sitePath, model.page), string.Empty, queryString);
                                return;
                            }
    ...
    public class SiteUrls
    {
        ...
        {
            SiteUrls _cache = CacheHelper.Get<SiteUrls>(PLKeys.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(PLKeys.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Utils.GetXmlMapPath(PLKeys.FILE_URL_XML_CONFING));
                    instance = CacheHelper.Get<SiteUrls>(PLKeys.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
        }
    ...
    public class SiteDomains
    {
        ...
        {
            SiteDomains _cache = CacheHelper.Get<SiteDomains>(PLKeys.CACHE_SITE_HTTP_DOMAIN);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(PLKeys.CACHE_SITE_HTTP_DOMAIN, new SiteDomains(), 10);
                    instance = CacheHelper.Get<SiteDomains>(PLKeys.CACHE_SITE_HTTP_DOMAIN);
                }
            }
            return instance;
        }

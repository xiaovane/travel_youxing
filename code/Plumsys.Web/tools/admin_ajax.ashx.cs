...
namespace Plumsys.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            string column_name = PLRequest.GetString("param");
        ...
        {
            string channel_name = PLRequest.GetString("param");
            string old_channel_name = PLRequest.GetString("old_channel_name");
        ...
        {
            string build_path = PLRequest.GetString("param");
            string old_build_path = PLRequest.GetString("old_build_path");
        ...
        {
            string new_name = PLRequest.GetString("param");
            string old_name = PLRequest.GetString("old_name");
        ...
        {
            string navname = PLRequest.GetString("param");
            string old_name = PLRequest.GetString("old_name");
        ...
        {
            string user_name = PLRequest.GetString("param");
        ...
        private void get_navigation_list(HttpContext context)
        {
            ...
            DataTable dt = new BLL.navigation().GetList(0, PLEnums.NavigationEnum.System.ToString());
        ...
        {
            string filePath = PLRequest.GetFormString("remotepath");
        ...
        {
            string mobiles = PLRequest.GetFormString("mobiles");
            string content = PLRequest.GetFormString("content");
            if (mobiles == "")
            {
        ...
        private void edit_order_status(HttpContext context)
        {
            ...
            string order_no = PLRequest.GetString("order_no");
            string edit_type = PLRequest.GetString("edit_type");
            if (order_no == "")
            {
            ...
            switch (edit_type.ToLower())
            {
                case "order_confirm": //确认订单
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", PLEnums.ActionEnum.Confirm.ToString()))
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Confirm.ToString(), "确认订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件============================
                ...
                case "order_payment": //确认付款
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Confirm.ToString(), "确认付款订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                ...
                case "order_express": //确认发货
                    ...
                    string express_no = PLRequest.GetFormString("express_no");
                    if (express_id == 0)
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Confirm.ToString(), "确认发货订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件============================
                ...
                case "order_complete": //完成订单=========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Confirm.ToString(), "确认交易完成订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件=========================
                ...
                case "order_cancel": //取消订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", PLEnums.ActionEnum.Cancel.ToString()))
                    {
                    ...
                    int check_revert1 = PLRequest.GetFormInt("check_revert");
                    if (check_revert1 == 1)
                    {
                        //如果存在积分换购则返还会员积分
                        if (model.user_id > 0 && model.point < 0)
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Cancel.ToString(), "取消订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"取消订单成功！\"}");
                    break;
                case "order_invalid": //作废订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", PLEnums.ActionEnum.Invalid.ToString()))
                    {
                    ...
                    int check_revert2 = PLRequest.GetFormInt("check_revert");
                    if (check_revert2 == 1)
                    {
                        //扣除购物赠送的积分
                        if (model.user_id > 0 && model.point > 0)
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Invalid.ToString(), "作废订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"作废订单成功！\"}");
                    break;
                case "edit_accept_info": //修改收货信息====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", PLEnums.ActionEnum.Edit.ToString()))
                    {
                    ...
                    string accept_name = PLRequest.GetFormString("accept_name");
                    string province = PLRequest.GetFormString("province");
                    string city = PLRequest.GetFormString("city");
                    string area = PLRequest.GetFormString("area");
                    string address = PLRequest.GetFormString("address");
                    string post_code = PLRequest.GetFormString("post_code");
                    string mobile = PLRequest.GetFormString("mobile");
                    string telphone = PLRequest.GetFormString("telphone");
                    string email = PLRequest.GetFormString("email");
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改收货信息，订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"修改收货人信息成功！\"}");
                    break;
                case "edit_order_remark": //修改订单备注=================================
                    string remark = PLRequest.GetFormString("remark");
                    if (remark == "")
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改订单备注，订单号:" + model.order_no); //记录日志
                ...
                case "edit_real_amount": //修改商品总金额================================
                    ...
                    decimal real_amount = PLRequest.GetFormDecimal("real_amount", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改商品金额，订单号:" + model.order_no); //记录日志
                ...
                case "edit_express_fee": //修改配送费用==================================
                    ...
                    decimal express_fee = PLRequest.GetFormDecimal("express_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改配送费用，订单号:" + model.order_no); //记录日志
                ...
                case "edit_payment_fee": //修改支付手续费=================================
                    ...
                    decimal payment_fee = PLRequest.GetFormDecimal("payment_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改支付手续费，订单号:" + model.order_no); //记录日志
                ...
                case "edit_invoice_taxes": //修改发票税金=================================
                    ...
                    decimal invoice_taxes = PLRequest.GetFormDecimal("invoice_taxes", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, PLEnums.ActionEnum.Edit.ToString(), "修改订单发票税金，订单号:" + model.order_no); //记录日志
        ...
        private int get_builder_status()
        {
            ...
            if (adminInfo == null)
                return -1;
            else if (!new BLL.manager_role().Exists(adminInfo.role_id, "sys_builder_html", PLEnums.ActionEnum.Build.ToString()))
                return -2;

...
namespace Plumsys.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void comment_add(HttpContext context)
        {
            ...
            string code = PLRequest.GetFormString("txtCode");
            int article_id = PLRequest.GetQueryInt("article_id");
        ...
        {
            int article_id = PLRequest.GetQueryInt("article_id");
            int page_index = PLRequest.GetQueryInt("page_index");
            int page_size = PLRequest.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
        ...
        {
            string username = PLRequest.GetString("param");
            //如果为Null，退出
        ...
        {
            string sitepath = PLRequest.GetQueryString("site");
            string username = PLRequest.GetFormString("txtUserName");
            string password = PLRequest.GetFormString("txtPassword");
            string remember = PLRequest.GetFormString("chkRemember");
            //检查站点目录
            ...
            context.Session[PLKeys.SESSION_USER_INFO] = model;
            context.Session.Timeout = 45;
            //记住登录状态下次自动登录
            if (remember.ToLower() == "true")
            {
                Utils.WriteCookie(PLKeys.COOKIE_USER_NAME_REMEMBER, "Plumsys", model.user_name, 43200);
                Utils.WriteCookie(PLKeys.COOKIE_USER_PWD_REMEMBER, "Plumsys", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(PLKeys.COOKIE_USER_NAME_REMEMBER, "Plumsys", model.user_name);
                Utils.WriteCookie(PLKeys.COOKIE_USER_PWD_REMEMBER, "Plumsys", model.password);
            }
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = PLRequest.GetString("txtUserName");
            string password = PLRequest.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
        ...
        private void user_oauth_register(HttpContext context)
        {
            ...
            string password = PLRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(PLRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(PLRequest.GetFormString("txtMobile").Trim());
            string userip = PLRequest.GetIP();
            //反序列化JSON
            Dictionary<string, object> dic = JsonHelper.DataRowFromJSON(result);
            if (dic["ret"].ToString() != "0")
            {
            ...
            else if (userConfig.regmsgstatus == 2) //发送邮件
            {
                ...
                if (mailModel != null)
                {
                    ...
                    //发送邮件
                    PLMail.sendMail(siteConfig.emailsmtp, siteConfig.emailssl, siteConfig.emailusername, siteConfig.emailpassword, siteConfig.emailnickname,
        ...
        {
            string site = PLRequest.GetQueryString("site").Trim(); //当前站点
            string code = PLRequest.GetFormString("txtCode").Trim();
            string username = Utils.ToHtml(PLRequest.GetFormString("txtUserName").Trim());
            string password = PLRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(PLRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(PLRequest.GetFormString("txtMobile").Trim());
            string userip = PLRequest.GetIP();
        ...
        {
            string mobile = Utils.ToHtml(PLRequest.GetString("mobile"));
            //检查手机
        ...
        {
            string site = PLRequest.GetString("site");
            string username = Utils.ToHtml(PLRequest.GetFormString("username"));
            ...
            //检查邮件是否过快
            string cookie = Utils.GetCookie(PLKeys.COOKIE_USER_EMAIL);
            if (cookie == username)
            {
            ...
            Utils.WriteCookie(PLKeys.COOKIE_USER_EMAIL, username, 20); //20分钟内无重复发送
            return;
        }
        #endregion
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string nick_name = Utils.ToHtml(PLRequest.GetFormString("txtNickName"));
            string sex = PLRequest.GetFormString("rblSex");
            string birthday = PLRequest.GetFormString("txtBirthday");
            string email = Utils.ToHtml(PLRequest.GetFormString("txtEmail"));
            string mobile = Utils.ToHtml(PLRequest.GetFormString("txtMobile"));
            string telphone = Utils.ToHtml(PLRequest.GetFormString("txtTelphone"));
            string qq = Utils.ToHtml(PLRequest.GetFormString("txtQQ"));
            string msn = Utils.ToHtml(PLRequest.GetFormString("txtMsn"));
            string province = Utils.ToHtml(PLRequest.GetFormString("txtProvince"));
            string city = Utils.ToHtml(PLRequest.GetFormString("txtCity"));
            string area = Utils.ToHtml(PLRequest.GetFormString("txtArea"));
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = PLRequest.GetFormString("hideFileName");
            int x1 = PLRequest.GetFormInt("hideX1");
            int y1 = PLRequest.GetFormInt("hideY1");
            int w = PLRequest.GetFormInt("hideWidth");
            int h = PLRequest.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = PLRequest.GetFormString("txtOldPassword");
            string password = PLRequest.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        {
            string site = PLRequest.GetQueryString("site");
            string code = PLRequest.GetFormString("txtCode");
            string type = PLRequest.GetFormString("txtType");
            string username = PLRequest.GetFormString("txtUserName").Trim();
            //检查站点目录
            if (string.IsNullOrEmpty(site))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，网站传输参数有误！\"}");
            ...
            if (type.ToLower() == "mobile") //使用手机取回密码
            {
                ...
                codeModel = codeBll.GetModel(username, PLEnums.CodeEnum.RegVerify.ToString(), "d");
                if (codeModel == null)
                {
                    ...
                    codeModel.type = PLEnums.CodeEnum.Password.ToString();
            ...
            else if (type.ToLower() == "email") //使用邮箱取回密码
            {
                ...
                try
                {
                    PLMail.sendMail(siteConfig.emailsmtp,
                        siteConfig.emailssl,
        ...
        {
            string strcode = PLRequest.GetFormString("hideCode"); //取回密码字符串
            string password = PLRequest.GetFormString("txtPassword"); //重设后的密码
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + PLEnums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + PLEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            string str_code = Utils.GetCheckCode(8);
            Model.user_code codeModel = new Model.user_code();
            codeModel.user_id = model.id;
            codeModel.user_name = model.user_name;
            codeModel.type = PLEnums.CodeEnum.Register.ToString();
            codeModel.str_code = str_code;
            codeModel.user_ip = PLRequest.GetIP();
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = PLRequest.GetFormInt("txtAmount");
            string password = PLRequest.GetFormString("txtPassword");
            if (model.amount < 1)
            {
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            string sitepath = PLRequest.GetQueryString("site"); //站点目录
            decimal amount = PLRequest.GetFormDecimal("order_amount", 0);
            int payment_id = PLRequest.GetFormInt("payment_id");
            //检查站点目录
            if (string.IsNullOrEmpty(sitepath))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"错误提示：站点传输参数不正确！\"}");
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string send_save = PLRequest.GetFormString("sendSave");
            string user_name = Utils.ToHtml(PLRequest.GetFormString("txtUserName"));
            string title = Utils.ToHtml(PLRequest.GetFormString("txtTitle"));
            string content = Utils.ToHtml(PLRequest.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
        ...
        private void user_address_edit(HttpContext context)
        {
            ...
            int id = PLRequest.GetQueryInt("id", 0);
            if (id > 0)
            {
            ...
            string code = PLRequest.GetFormString("txtCode");
            string accept_name = PLRequest.GetFormString("txtAcceptName");
            string province = Utils.ToHtml(PLRequest.GetFormString("txtProvince"));
            string city = Utils.ToHtml(PLRequest.GetFormString("txtCity"));
            string area = Utils.ToHtml(PLRequest.GetFormString("txtArea"));
            string address = Utils.ToHtml(context.Request.Form["txtAddress"]);
            ...
            string telphone = Utils.ToHtml(PLRequest.GetFormString("txtTelphone"));
            string email = Utils.ToHtml(PLRequest.GetFormString("txtEmail"));
            string post_code = Utils.ToHtml(PLRequest.GetFormString("txtPostCode"));
        ...
        private void user_point_delete(HttpContext context)
        {
            ...
            string check_id = PLRequest.GetFormString("checkId");
        ...
        {
            string jsonData = PLRequest.GetFormString("jsondata");
        ...
        {
            int clear = PLRequest.GetFormInt("clear", 0);
            int article_id = PLRequest.GetFormInt("article_id", 0);
            int goods_id = PLRequest.GetFormInt("goods_id", 0);
            if (clear == 1)
            {
        ...
        {
            //获取传参信息===================================
            string hideGoodsJson = Utils.GetCookie(PLKeys.COOKIE_SHOPPING_BUY); //获取商品JSON数据
            string sitepath = PLRequest.GetQueryString("site"); //站点目录
            int book_id = PLRequest.GetFormInt("book_id", 1);
            int payment_id = PLRequest.GetFormInt("payment_id");
            int express_id = PLRequest.GetFormInt("express_id");
            int is_invoice = PLRequest.GetFormInt("is_invoice", 0);
            string accept_name = Utils.ToHtml(PLRequest.GetFormString("accept_name"));
            string province = Utils.ToHtml(PLRequest.GetFormString("province"));
            string city = Utils.ToHtml(PLRequest.GetFormString("city"));
            string area = Utils.ToHtml(PLRequest.GetFormString("area"));
            string address = Utils.ToHtml(PLRequest.GetFormString("address"));
            string telphone = Utils.ToHtml(PLRequest.GetFormString("telphone"));
            string mobile = Utils.ToHtml(PLRequest.GetFormString("mobile"));
            string email = Utils.ToHtml(PLRequest.GetFormString("email"));
            string post_code = Utils.ToHtml(PLRequest.GetFormString("post_code"));
            string message = Utils.ToHtml(PLRequest.GetFormString("message"));
            string invoice_title = Utils.ToHtml(PLRequest.GetFormString("invoice_title"));
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = PLRequest.GetQueryString("order_no");
        ...
        {
            int article_id = PLRequest.GetInt("id", 0);
            int click = PLRequest.GetInt("click", 0);
            int hide = PLRequest.GetInt("hide", 0);
            int count = 0;
            if (article_id > 0)
            {
        ...
        private void view_attach_count(HttpContext context)
        {
            ...
            string view = PLRequest.GetString("view");
            int count = 0;
            if (id > 0)
            {
        ...
        private string verify_code(HttpContext context, string strcode)
        {
            ...
            if (strcode.ToLower() != (context.Session[PLKeys.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[PLKeys.SESSION_CODE] = null;
            return "success";
        }
        #endregion
        ...
        private string verify_sms_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[PLKeys.SESSION_SMS_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[PLKeys.SESSION_SMS_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[PLKeys.SESSION_SMS_CODE] = null;
            return "success";
        }
        #endregion
        ...
        private string send_verify_sms_code(HttpContext context,string mobile)
        {
            ...
            //检查是否过期
            string cookie = Utils.GetCookie(PLKeys.COOKIE_USER_MOBILE);
            if (cookie == mobile)
            {
            ...
            //写入SESSION，保存验证码
            context.Session[PLKeys.SESSION_SMS_CODE] = strcode;
            Utils.WriteCookie(PLKeys.COOKIE_USER_MOBILE, mobile, userConfig.regsmsexpired); //规定时间内无重复发送
            return "success";
        }
        #endregion
        ...
        private string send_verify_email(string site, Model.users userModel)
        {
            ...
            Model.user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, PLEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = PLEnums.CodeEnum.RegVerify.ToString();
                codeModel.str_code = strcode;
                codeModel.eff_time = DateTime.Now.AddDays(userConfig.regemailexpired);
                codeModel.add_time = DateTime.Now;
            ...
            try
            {
                PLMail.sendMail(siteConfig.emailsmtp, siteConfig.emailssl,
                    siteConfig.emailusername,
                    DESEncrypt.Decrypt(siteConfig.emailpassword),
                    siteConfig.emailnickname,
                    siteConfig.emailfrom,

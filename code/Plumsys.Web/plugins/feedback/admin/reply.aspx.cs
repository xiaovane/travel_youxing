...
{
    public partial class reply : Plumsys.Web.UI.ManagePage
    {
        private int id = 0;
        ...
        {
            ChkAdminLevel("plugin_feedback", PLEnums.ActionEnum.Reply.ToString()); //检查权限
            BLL.feedback bll = new BLL.feedback();
            model = bll.GetModel(this.id);
            model.reply_content = Utils.ToHtml(txtReContent.Text);
            model.reply_time = DateTime.Now;
            bll.Update(model);
            AddAdminLog(PLEnums.ActionEnum.Reply.ToString(), "回复留言插件内容：" + model.title); //记录日志

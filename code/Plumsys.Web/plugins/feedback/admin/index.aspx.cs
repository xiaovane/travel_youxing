...
namespace Plumsys.Web.Plugin.Feedback.admin
{
    public partial class index : Plumsys.Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("feedback_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("feedback_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("plugin_feedback", PLEnums.ActionEnum.Audit.ToString()); //检查权限
            BLL.feedback bll = new BLL.feedback();
            ...
            AddAdminLog(PLEnums.ActionEnum.Audit.ToString(), "审核留言插件内容"); //记录日志
        ...
        {
            ChkAdminLevel("plugin_feedback", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.feedback bll = new BLL.feedback();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除留言成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

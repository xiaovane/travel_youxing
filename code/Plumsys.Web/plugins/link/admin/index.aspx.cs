...
namespace Plumsys.Web.Plugin.Link.admin
{
    public partial class index : Plumsys.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("plugin_link", PLEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("link_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            ChkAdminLevel("plugin_link", PLEnums.ActionEnum.Edit.ToString()); //检查权限
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("link_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        protected void lbtnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改友情链接插件排序:"); //记录日志
        ...
        {
            ChkAdminLevel("plugin_link", PLEnums.ActionEnum.Audit.ToString()); //检查权限
            BLL.link bll = new BLL.link();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                int id = Convert.ToInt32(((HiddenField)rptList.Items[i].FindControl("hidId")).Value);
            ...
            AddAdminLog(PLEnums.ActionEnum.Audit.ToString(), "审核友情链接"); //记录日志
        ...
        {
            ChkAdminLevel("plugin_link", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.link bll = new BLL.link();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除友情链接成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

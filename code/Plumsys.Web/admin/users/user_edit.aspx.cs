...
namespace Plumsys.Web.admin.users
{
    public partial class user_edit : Web.UI.ManagePage
    {
        ...
        protected string action = PLEnums.ActionEnum.Add.ToString(); //操作类型
        private int id = 0;
        ...
        private bool DoAdd()
        {
            ...
            model.reg_ip = PLRequest.GetIP();

            if (bll.Add(model) > 0)
            {
                AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "添加用户:" + model.user_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改用户信息:" + model.user_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == PLEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("user_list", PLEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("user_list", PLEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;

...
namespace Plumsys.Web.admin.users
{
    public partial class user_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_config", PLEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("user_config", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.userconfig bll = new BLL.userconfig();
            Model.userconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改用户配置信息"); //记录日志

...
namespace Plumsys.Web.admin.users
{
    public partial class group_list : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("user_group", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_groups bll = new BLL.user_groups();
            //批量删除
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除用户组成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

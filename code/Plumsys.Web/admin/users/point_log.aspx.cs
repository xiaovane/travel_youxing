...
namespace Plumsys.Web.admin.users
{
    public partial class point_log : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_point_log_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_point_log_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_point_log", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_point_log bll = new BLL.user_point_log();
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除积分日志成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

...
namespace Plumsys.Web.admin.users
{
    public partial class user_audit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_audit", PLEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_audit_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_audit_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_audit", PLEnums.ActionEnum.Audit.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.users bll = new BLL.users();
            ...
            AddAdminLog(PLEnums.ActionEnum.Audit.ToString(), "审核会员成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

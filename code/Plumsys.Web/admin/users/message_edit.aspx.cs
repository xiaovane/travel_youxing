...
namespace Plumsys.Web.admin.users
{
    public partial class message_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == PLEnums.ActionEnum.View.ToString())
            {
                this.action = PLEnums.ActionEnum.View.ToString();//修改类型
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (action == PLEnums.ActionEnum.View.ToString()) //修改
                {
                    ShowInfo(this.id);
                }
            }
        ...
        {
            ChkAdminLevel("user_message", PLEnums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
                JscriptMsg("发送过程中发生错误！", "");
                return;
            }
            AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "发送站内短消息"); //记录日志

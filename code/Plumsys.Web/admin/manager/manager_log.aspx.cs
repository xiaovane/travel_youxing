...
namespace Plumsys.Web.admin.manager
{
    public partial class manager_log : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("manager_log", PLEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("manager_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("manager_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("manager_log", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            BLL.manager_log bll = new BLL.manager_log();
            int sucCount = bll.Delete(7);
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除管理日志" + sucCount + "条"); //记录日志

...
namespace Plumsys.Web.admin.order
{
    public partial class order_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_config", PLEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("order_config", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.orderconfig bll = new BLL.orderconfig();
            Model.orderconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改订单配置信息"); //记录日志

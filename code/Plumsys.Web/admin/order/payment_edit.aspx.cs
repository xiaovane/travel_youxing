...
namespace Plumsys.Web.admin.order
{
    public partial class payment_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_payment", PLEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo(this.id);
            }
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改支付方式:" + model.title); //记录日志
                result = true;
            }
        ...
        {
            ChkAdminLevel("order_payment", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            if (!DoEdit(this.id))
            {

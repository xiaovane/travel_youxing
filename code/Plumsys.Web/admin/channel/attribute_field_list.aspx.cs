...
namespace Plumsys.Web.admin.channel
{
    public partial class attribute_field_list : Web.UI.ManagePage
    {
        ...
        {
            this.control_type = PLRequest.GetQueryString("control_type");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("attribute_field_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("attribute_field_page_size", "PlumsysPage", _pagesize.ToString(), 43200);
                }
            }
        ...
        {
            ChkAdminLevel("sys_channel_field", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.article_attribute_field bll = new BLL.article_attribute_field();
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "保存扩展字段排序"); //记录日志
        ...
        {
            ChkAdminLevel("sys_channel_field", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0; //成功数量
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除扩展字段成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

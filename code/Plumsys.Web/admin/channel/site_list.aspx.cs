...
namespace Plumsys.Web.admin.channel
{
    public partial class site_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("channel_site_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("channel_site_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("sys_site_manage", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.channel_site bll = new BLL.channel_site();
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "保存站点排序"); //记录日志
        ...
        {
            ChkAdminLevel("sys_site_manage", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.channel_site bll = new BLL.channel_site();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    {
                        sucCount += 1;
                        Utils.DeleteDirectory(siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                        Utils.DeleteDirectory(siteConfig.webpath + PLKeys.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                    }
                    else
                    {
                        errorCount += 1;
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除站点成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

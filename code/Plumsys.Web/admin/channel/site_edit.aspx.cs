...
namespace Plumsys.Web.admin.channel
{
    public partial class site_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            {
                //更新一下域名缓存
                CacheHelper.Remove(PLKeys.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "添加站点:" + model.title); //记录日志
                return true;
            }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                //更新一下域名缓存
                CacheHelper.Remove(PLKeys.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改站点:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("sys_site_manage", PLEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;

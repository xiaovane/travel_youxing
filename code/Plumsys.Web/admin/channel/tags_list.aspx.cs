...
namespace Plumsys.Web.admin.channel
{
    public partial class tags_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_article_tags", PLEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("tags_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("tags_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("sys_article_tags", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.article_tags bll = new BLL.article_tags();
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "保存Tags标签排序"); //记录日志
        ...
        {
            ChkAdminLevel("sys_article_tags", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            BLL.article_tags bll = new BLL.article_tags();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                int id = Convert.ToInt32(((HiddenField)rptList.Items[i].FindControl("hidId")).Value);
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除Tags标签"); //记录日志

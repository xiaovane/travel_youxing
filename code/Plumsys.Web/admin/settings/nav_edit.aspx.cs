    ...
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.nav_type = PLEnums.NavigationEnum.System.ToString();
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "添加导航菜单:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "修改导航菜单:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == PLEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_navigation", PLEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("sys_navigation", PLEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;

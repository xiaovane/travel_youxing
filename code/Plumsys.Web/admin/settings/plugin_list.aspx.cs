...
namespace Plumsys.Web.admin.settings
{
    public partial class plugin_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("sys_plugin_config", PLEnums.ActionEnum.View.ToString()); //检查权限
                RptBind();
            }
        }
        ...
        {
            //插件目录
            string pluginPath = Utils.GetMapPath("../../plugins/" + dirName + "/" + PLKeys.FILE_PLUGIN_XML_CONFING);
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite" && xe.Attributes["page"] != null)
                    {
                        ...
                        {
                            //删除站点下的aspx文件
                            Utils.DeleteFile(siteConfig.webpath + "aspx/" + PLKeys.DIRECTORY_REWRITE_PLUGIN + "/" + xe.Attributes["page"].Value);
                        }
                    }
                }
            }
        ...
        {
            ChkAdminLevel("sys_plugin_config", PLEnums.ActionEnum.Instal.ToString()); //检查权限
            //插件目录
            ...
            AddAdminLog(PLEnums.ActionEnum.Instal.ToString(), "安装插件"); //记录日志
        ...
        {
            ChkAdminLevel("sys_plugin_config", PLEnums.ActionEnum.UnLoad.ToString()); //检查权限
            //插件目录
            string pluginPath = Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(PLEnums.ActionEnum.UnLoad.ToString(), "卸载插件"); //记录日志
        ...
        {
            ChkAdminLevel("sys_plugin_config", PLEnums.ActionEnum.Build.ToString()); //检查权限
            //插件目录
            string pluginPath = Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(PLEnums.ActionEnum.Build.ToString(), "生成插件模板"); //记录日志

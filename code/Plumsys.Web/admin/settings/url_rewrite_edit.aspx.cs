...
namespace Plumsys.Web.admin.settings
{
    public partial class url_rewrite_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model))
            {
                AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "添加URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(string _urlName)
        {
            ...
            if (bll.Edit(model))
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == PLEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_url_rewrite", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            ...
            {
                ChkAdminLevel("sys_url_rewrite", PLEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;

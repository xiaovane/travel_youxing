<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="article_edit.aspx.cs" Inherits="Plumsys.Web.admin.article.article_edit" ValidateRequest="false" %>
...
<head>
    ...
    <link href="../../scripts/pricepicker/css/pricePicker.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript" charset="utf-8" src="../../scripts/pricepicker/js/pricePicker.js"></script>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            //初始化页面窗口
            $(".spec-pirce-btn").click(function () {
                $.post("../../tools/admin_ajax.ashx?action=get_article_goods", { "id": $.request("id") }, function (data) {
        ...
        //初始化日期规格窗口
        function showPriceSpecDialog(date) {
            var d = top.dialog({
                id: 'specPriceDialogId',
                padding: 0,
                width: 1000,
                height: 600,
                title: "商品日期规格",
                url: 'dialog/dialog_goods_price.aspx?date=' + date + '&article_id=' + $.request("id")
            }).showModal();
            //将容器对象传进去
            d.data = $(".spec-item");
        }
        ...
        //绑定已选择规格
        function bindSpec() {
            var val = $("#<%=hide_goods_spec_list.ClientID%>").val();
            var json_spec_list = "";
            if (val == "") {
                json_spec_list = jQuery.parseJSON("[" + val + "]");
            }
            else json_spec_list = jQuery.parseJSON(val);
            for (var i = 0; i < json_spec_list.length; i++) {
                $(".spec-item li").each(function () {
                    if (!$(this).hasClass("selected") && $(this).attr("id") == json_spec_list[i].spec_id) {
        ...
        function createSpecJson() {
            ...
            //赋值给父对象的隐藏域
            $("#<%=hide_goods_spec_list.ClientID%>").val("[" + goodsSpecList + "]");
            //生成货品的笛卡尔积
            var specMaxData = descartes(specValueData, specData);
            var trHtml = ''; //规格总HTML
            var objProductNo = $("#field_control_goods_no").val()==""?$("#item_box").find("input[name='spec_goods_no']").last().length == 0?"":$("#item_box").find("input[name='spec_goods_no']").last().val().split('-')[0]:$("#field_control_goods_no").val();
            var objProductIndex = $("#item_box").find("input[name='spec_goods_no']").last().length == 0 ? 0 : $("#item_box").find("input[name='spec_goods_no']").last().val().split('-')[1];
            var index_No = 1;
            ...
            for (var i = 0; i < specMaxData.length; i++) {
                ...
                var hide_goods_id = '0';
                var spec_goods_no = defaultProductNo + '-' +(parseInt(objProductIndex)+index_No);
                var spec_market_price = '0';
                var spec_sell_price = '0';
                var spec_sell_date = new Date().Format('yyyy-MM-dd');
    ...
    <form id="form1" runat="server">
      ...
      <asp:Label ID="div_sell_price_tip" runat="server" CssClass="Validform_checktip" />
            ...
            <dl id="div_spec__container" runat="server" visible="false">
                <dt>商品规格</dt>
                <dd>
                    <a class="icon-btn add spec-pirce-btn"><span>设置规格</span></a>
                    <div id="calendar"></div>

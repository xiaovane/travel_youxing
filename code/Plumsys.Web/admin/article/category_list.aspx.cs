...
namespace Plumsys.Web.admin.article
{
    public partial class category_list : Web.UI.ManagePage
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "保存" + this.channel_name + "频道栏目分类排序"); //记录日志
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_category", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            BLL.article_category bll = new BLL.article_category();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                int id = Convert.ToInt32(((HiddenField)rptList.Items[i].FindControl("hidId")).Value);
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "删除" + this.channel_name + "频道栏目分类数据"); //记录日志

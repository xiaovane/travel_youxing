...
namespace Plumsys.Web.admin.article
{
    public partial class comment_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("article_comment_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("article_comment_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("comment_list.aspx", "channel_id={0}&keywords={1}&property={2}",
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_comment", PLEnums.ActionEnum.Audit.ToString()); //检查权限
            BLL.article_comment bll = new BLL.article_comment();
            ...
            AddAdminLog(PLEnums.ActionEnum.Audit.ToString(), "审核" + this.channel_name + "频道评论信息"); //记录日志
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_comment", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.article_comment bll = new BLL.article_comment();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除" + this.channel_name + "频道评论成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

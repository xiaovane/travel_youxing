					...
					if(typeof(liObj.children("a").attr("href"))!="undefined"){
						...
						liObj.children("a").click(function () {
							...
							if(typeof($(this).attr("navid"))!="undefined"){
								addCookie("pl_manage_navigation_cookie", $(this).attr("navid"), 240);
							}
						});
					}
				}
...
function linkMenuTree(islink, navid) {
	...
	if (argument == 2) {
		...
		cookieObj = navGroupObj.find('a[navid="' + getCookie("pl_manage_navigation_cookie") + '"]');
	}
	if (cookieObj.length > 0) {
		...
		//检查是否需要保存到cookie
		if (argument == 2) {
			addCookie("pl_manage_navigation_cookie", navid, 240);
		}
		//检查是否需要跳转链接
		...
		navGroupObj.find("ul li a").removeClass("selected");
		//保存到cookie
		addCookie("pl_manage_navigation_cookie", "", 240);
	}
}

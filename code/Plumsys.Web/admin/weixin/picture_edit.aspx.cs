...
namespace Plumsys.Web.admin.weixin
{
    public partial class picture_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(PLEnums.ActionEnum.Add.ToString(), "添加微信图文回复，关健字:" + model.keywords); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "修改微信图文回复，关健字:" + model.keywords); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("weixin_response_picture", PLEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", string.Empty);
                    return;

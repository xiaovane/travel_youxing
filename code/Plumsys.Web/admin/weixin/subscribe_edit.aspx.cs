...
namespace Plumsys.Web.admin.weixin
{
    public partial class subscribe_edit : Web.UI.ManagePage
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                case "default":
                    ChkAdminLevel("weixin_subscribe_default", PLEnums.ActionEnum.Edit.ToString());
                    break;
                case "cancel":
                    ChkAdminLevel("weixin_subscribe_cancel", PLEnums.ActionEnum.Edit.ToString());
                    break;
                default:
                    ChkAdminLevel("weixin_subscribe_subscribe", PLEnums.ActionEnum.Edit.ToString());
                    break;
            }
            //开始保存数据
            ...
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "新增微信" + ruleName); //记录日志
            ...
            {
                AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "编辑微信" + ruleName); //记录日志
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                case "default":
                    ChkAdminLevel("weixin_subscribe_default", PLEnums.ActionEnum.Delete.ToString());

                    break;
                case "cancel":
                    ChkAdminLevel("weixin_subscribe_cancel", PLEnums.ActionEnum.Delete.ToString());

                    break;
                default:
                    ChkAdminLevel("weixin_subscribe_subscribe", PLEnums.ActionEnum.Delete.ToString());
            ...
            if (ruleModel != null)
            {
                ...
                if (result)
                {
                    AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除" + ruleModel.name + "，公众账户ID：" + ddlAccountId.SelectedValue); //记录日志

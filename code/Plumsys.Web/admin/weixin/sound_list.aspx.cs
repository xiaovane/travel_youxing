...
namespace Plumsys.Web.admin.weixin
{
    public partial class sound_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("weixin_response_sound_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("weixin_response_sound_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("weixin_response_sound", PLEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.weixin_request_rule bll = new BLL.weixin_request_rule();
            ...
            AddAdminLog(PLEnums.ActionEnum.Edit.ToString(), "保存微信语音回复排序"); //记录日志
        ...
        {
            ChkAdminLevel("weixin_response_sound", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.weixin_request_rule bll = new BLL.weixin_request_rule();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除微信语音回复成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

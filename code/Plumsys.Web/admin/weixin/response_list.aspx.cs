...
namespace Plumsys.Web.admin.weixin
{
    public partial class response_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("weixin_response_content", PLEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("weixin_response_content_page_size", "PlumsysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("weixin_response_content_page_size", "PlumsysPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("weixin_response_content", PLEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.weixin_response_content bll = new BLL.weixin_response_content();
            ...
            AddAdminLog(PLEnums.ActionEnum.Delete.ToString(), "删除微信请求回复" + sucCount + "条，失败" + errorCount + "条"); //记录日志
